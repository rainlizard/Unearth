...
func add_object(thingType, subtype, array):
	# Initialize a new dictionary for subtypes if thingType hasn't been added before.
	if CUSTOM_OBJECTS.has(thingType) == false:
		CUSTOM_OBJECTS[thingType] = {}
	
	CUSTOM_OBJECTS[thingType][subtype] = array
